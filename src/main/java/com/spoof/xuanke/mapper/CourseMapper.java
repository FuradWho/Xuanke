...
public interface CourseMapper extends BaseMapper<Course> {
    ...
    @Select({
            ...
            "t_course.course_id,t_course.course_credit,t_course.course_cycle,t_course.course_name,t_course_type.name,t_student.stu_name",
            "FROM t_course,t_course_type,t_student",
            "WHERE t_course.course_teacher_id=t_student.stu_id and t_course.courseType_id = t_course_type.id",
            "limit #{startIndex},#{pageSize}",
            "</script>"
    })
    List<GetCourse> getAllCourse(@Param("startIndex") int startIndex, @Param("pageSize") int pageSize);
